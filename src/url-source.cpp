...
static void url_source_destroy(void *data)
{
	struct url_source_data *usd =
		reinterpret_cast<struct url_source_data *>(data);
...
void curl_loop(struct url_source_data *usd)
{
	...
	while (true) {
		...
		if (response.status_code != 200) {
			...
			render_text_with_qtextdocument(response.body_parsed,
						       width, height,
						       &renderBuffer);
			// Update the frame
		...
		std::this_thread::sleep_for(
			std::chrono::milliseconds(usd->update_timer_ms));
	}
...
{
	struct url_source_data *usd =
		reinterpret_cast<struct url_source_data *>(
			bzalloc(sizeof(struct url_source_data)));
	usd->source = source;

	// get request data from settings
	std::string serialized_request_data =
		obs_data_get_string(settings, "request_data");
	if (serialized_request_data.empty()) {
		...
		std::string serialized_request_data =
			serialize_request_data(&(usd->request_data));
		// Save on settings
		...
		usd->request_data =
			unserialize_request_data(serialized_request_data);
	}
...
static void url_source_update(void *data, obs_data_t *settings)
{
	...
	usd->update_timer_ms =
		(uint32_t)obs_data_get_int(settings, "update_timer");
...
static obs_properties_t *url_source_properties(void *data)
{
	...
	obs_property_t *urlprop =
		obs_properties_add_text(ppts, "url", "URL", OBS_TEXT_DEFAULT);
	...
	obs_properties_add_int(ppts, "update_timer", "Update Timer (ms)", 100,
			       10000, 100);
...
struct obs_source_info url_source {
	.id = "url_source", .type = OBS_SOURCE_TYPE_INPUT,
	.output_flags = OBS_SOURCE_ASYNC_VIDEO, .get_name = url_source_name,
	.create = url_source_create, .destroy = url_source_destroy,
	.get_defaults = url_source_defaults,
	.get_properties = url_source_properties, .update = url_source_update
};
