...
public:
  ...
  void populateFromPairs(std::vector<std::pair<std::string, std::string>> &pairs) {
    for (auto &pair : pairs) {
      ...
      QLineEdit *valueLineEdit = (QLineEdit *)keyValueWidget->layout()->itemAt(1)->widget();
...
void get_key_value_as_pairs_from_key_value_list_widget(KeyValueListWidget *widget,
                   std::vector<std::pair<std::string, std::string>> &pairs)
{
  pairs.clear();
  for (int i = 0; i < widget->layout()->count() - 1; i++) {
    KeyValueWidget *keyValueWidget = (KeyValueWidget *)widget->layout()->itemAt(i)->widget();
    QLineEdit *keyLineEdit = (QLineEdit *)keyValueWidget->layout()->itemAt(0)->widget();
    QLineEdit *valueLineEdit = (QLineEdit *)keyValueWidget->layout()->itemAt(1)->widget();
    pairs.push_back(std::make_pair(keyLineEdit->text().toStdString(), valueLineEdit->text().toStdString()));
  }
}

RequestBuilder::RequestBuilder(url_source_request_data *request_data, std::function<void()> update_handler, QWidget *parent)
	: QDialog(parent), layout(new QVBoxLayout)
{
  ...
  auto saveSettingsToRequestData = [=](url_source_request_data *request_data_for_saving) {
    ...
    // Save the headers
    get_key_value_as_pairs_from_key_value_list_widget(headersWidget, request_data_for_saving->headers);

    // Save the output parsing options
    request_data_for_saving->output_type = outputTypeComboBox->currentText().toStdString();
    request_data_for_saving->output_json_path = outputJSONPathLineEdit->text().toStdString();
    ...
    request_data_for_saving->output_regex_flags = outputRegexFlagsLineEdit->text().toStdString();
    request_data_for_saving->output_regex_group = outputRegexGroupLineEdit->text().toStdString();
  };
  ...
  connect(sendButton, &QPushButton::clicked, this, [=]() {
    ...
    // Send the request
    struct request_data_handler_response response = request_data_handler(request_data_test);
