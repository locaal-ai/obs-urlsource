...
void curl_loop(struct url_source_data *usd) {
  ...
  while (true) {
    ...
    // Send the request
    struct request_data_handler_response response = request_data_handler(&(usd->request_data));
    if (response.status_code != 200) {
      obs_log(LOG_INFO, "Failed to send request: %s", response.error_message.c_str());
    } else {
      ...
      render_text_with_qtextdocument(response.body_parsed, width, height, &renderBuffer);
      // Update the frame
...
{
	struct url_source_data *usd = reinterpret_cast<struct url_source_data *>(bzalloc(sizeof(struct url_source_data)));
	usd->source = source;
...
static obs_properties_t *url_source_properties(void *data)
{
  ...
  obs_properties_add_button2(ppts, "button", "Setup Request", [](obs_properties_t*,
    ...
    RequestBuilder *builder = new RequestBuilder(&(button_usd->request_data),
      [=]() {
        // Update the request data from the settings
        obs_data_t *settings = obs_source_get_settings(button_usd->source);
        // serialize request data
        std::string serialized_request_data = serialize_request_data(&(button_usd->request_data));
        // Save on settings
        obs_data_set_string(settings, "request_data", serialized_request_data.c_str());
