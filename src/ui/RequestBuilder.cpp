...
public:
	KeyValueListWidget(QWidget *parent = nullptr)
		: QWidget(parent), listLayout(new QVBoxLayout)
	{
		...
		connect(addButton, &QPushButton::clicked, this, [=]() {
			...
			KeyValueWidget *keyValueWidget =
				new KeyValueWidget(this);
			listLayout->insertWidget(listLayout->count() - 1,
						 keyValueWidget);
		});
	}

	void populateFromPairs(
		std::vector<std::pair<std::string, std::string>> &pairs)
	{
		for (auto &pair : pairs) {
			KeyValueWidget *keyValueWidget = new KeyValueWidget;
			QLineEdit *keyLineEdit =
				(QLineEdit *)keyValueWidget->layout()
					->itemAt(0)
					->widget();
	...
	std::vector<std::pair<std::string, std::string>> &pairs)
{
	...
	for (int i = 0; i < widget->layout()->count() - 1; i++) {
		...
		QLineEdit *keyLineEdit = (QLineEdit *)keyValueWidget->layout()
						 ->itemAt(0)
						 ->widget();
		QLineEdit *valueLineEdit = (QLineEdit *)keyValueWidget->layout()
						   ->itemAt(1)
						   ->widget();
		pairs.push_back(
			std::make_pair(keyLineEdit->text().toStdString(),
	...
	: QDialog(parent), layout(new QVBoxLayout)
{
	...
	// set value from request_data
	methodComboBox->setCurrentText(
		QString::fromStdString(request_data->method));
	...
	outputJSONPathLineEdit->setText(
		QString::fromStdString(request_data->output_json_path));
	formOutputParsing->addRow("JSON Pointer", outputJSONPathLineEdit);
	QLineEdit *outputXPathLineEdit = new QLineEdit;
	outputXPathLineEdit->setText(
		QString::fromStdString(request_data->output_xpath));
	...
	outputRegexLineEdit->setText(
		QString::fromStdString(request_data->output_regex));
	...
	outputRegexFlagsLineEdit->setText(
	...
	outputRegexGroupLineEdit->setText(
	...
	connect(outputTypeComboBox, &QComboBox::currentTextChanged, this, [=]() {
		if (outputTypeComboBox->currentText() == "JSON") {
			...
			formOutputParsing->setRowVisible(
				outputRegexFlagsLineEdit, false);
			formOutputParsing->setRowVisible(
				outputRegexGroupLineEdit, false);
		}
		...
		if (outputTypeComboBox->currentText() == "Text") {
			...
			formOutputParsing->setRowVisible(outputRegexLineEdit,
							 true);
			formOutputParsing->setRowVisible(
				outputRegexFlagsLineEdit, true);
			formOutputParsing->setRowVisible(
				outputRegexGroupLineEdit, true);
		}
	});
	...
	auto saveSettingsToRequestData =
		[=](url_source_request_data *request_data_for_saving) {
			// Save the request settings to the request_data struct
			request_data_for_saving->url =
				urlLineEdit->text().toStdString();
			request_data_for_saving->method =
				methodComboBox->currentText().toStdString();
			request_data_for_saving->body =
				bodyLineEdit->text().toStdString();

			// Save the headers
			get_key_value_as_pairs_from_key_value_list_widget(
				headersWidget,
			...
			request_data_for_saving->output_xpath =
				outputXPathLineEdit->text().toStdString();
			request_data_for_saving->output_regex =
				outputRegexLineEdit->text().toStdString();
	...
	connect(sendButton, &QPushButton::clicked, this, [=]() {
		...
		QLabel *responseLabel =
			new QLabel(QString::fromStdString(response.body));
		// Wrap the text
		...
		if (response.body_parsed != "") {
			QGroupBox *parsedOutputGroupBox =
				new QGroupBox("Parsed Output");
			...
			parsedOutputLayout->addWidget(new QLabel(
				QString::fromStdString(response.body_parsed)));
		}
